...
class AwsEventStreamParser:
    ...
    EVENT_PATTERNS = [
        ('{"content":', 'content'),
        ('{"toolUseId":', 'tool_start'),  # Kiro sends toolUseId+name+input together
        ('{"name":', 'tool_start'),
        ('{"input":', 'tool_input'),
        ('{"stop":', 'tool_stop'),
    ...
    def _process_tool_start_event(self, data: dict) -> Optional[Dict[str, Any]]:
        ...
        return {"type": "tool_start", "data": {"id": tool_id, "name": tool_name, "input": input_str}}
    ...
    def _process_tool_input_event(self, data: dict) -> Optional[Dict[str, Any]]:
        ...
        # Kiro sends cumulative snapshots: new input starts with old input minus closing '}'
        if current_args and len(current_args) > 1 and input_str.startswith(current_args[:-1]):
            delta = input_str[len(current_args) - 1:]
            self.current_tool_call['function']['arguments'] = input_str
        else:
            delta = input_str
            self.current_tool_call['function']['arguments'] += input_str

        return {"type": "tool_input", "data": {"input": delta}}
