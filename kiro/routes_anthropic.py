...
async def messages(
    ...
    if is_codex_model(request_data.model):
        ...
        if request_data.stream:
            async def codex_stream_wrapper():
                try:
                    async for chunk in stream_codex_response(request_dict, request_data.model):
                        yield chunk
                except HTTPException as e:
                    yield f'event: error\ndata: {json.dumps(e.detail)}\n\n'
                except Exception as e:
                    logger.error(f"Codex streaming error: {e}", exc_info=True)
                    yield f'event: error\ndata: {json.dumps({"type": "error", "error": {"type": "api_error", "message": str(e)}})}\n\n'

            return StreamingResponse(
                codex_stream_wrapper(),
            ...
            try:
                ...
                logger.error(f"Codex non-streaming error: {e}", exc_info=True)
