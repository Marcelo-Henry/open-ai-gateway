...
```mermaid
C4Context
    title Kiro Gateway — Diagrama de Contexto

    Person(dev, "Desenvolvedor / Agente AI", "Usa Claude Code, Cursor, ou outro cliente compatível com OpenAI/Anthropic API")

    System(gateway, "Kiro Gateway", "Proxy FastAPI que traduz requisições OpenAI/Anthropic para o formato nativo da Kiro API. Gerencia autenticação, streaming, failover multi-account e enriquecimentos opcionais.")
